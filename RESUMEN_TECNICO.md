...
**Sobre `labels` (idioma/acento/género/edad) y grabar varias muestras:** se investigó si valía la pena implementarlos. La documentación de ElevenLabs es clara en dos puntos: (1) `labels` es metadata para organizar/filtrar voces en el panel de ElevenLabs, **no afecta la calidad del clon**; (2) para Instant Voice Cloning "el número de muestras no importa, lo que importa es la duración total combinada" — duplicar la misma grabación en varios archivos no aporta nada (es el mismo contenido repetido). Por eso la app sí completa `labels` (accent fijo en `"Latino (Ecuador)"`, e `language` detectado automáticamente con Whisper sobre la primera muestra, con `response_format="verbose_json"`) porque es prácticamente gratis y ayuda a organizar el panel, pero **no** se implementó "grabar varias muestras" ni se agregaron campos de género/edad al formulario (el guion sugerido ya cumple el rol de aportar variedad dentro de una misma grabación, sin la fricción de pedirle datos extra al usuario para una etiqueta que no cambia el audio).
...
**¿Por qué no se usaron otros servicios que hacen lo mismo? Dame ejemplos.**
Para cada capacidad existen alternativas reales, y se evaluaron conceptualmente antes de elegir:

| Capacidad | Alternativas que existen | Por qué se eligió lo que usa la app |
|---|---|---|
| Mejorar redacción (LLM) | Anthropic Claude, Google Gemini, modelos open-source autoalojados (Llama, Mistral) | OpenAI ya era necesario para transcribir audio (Whisper); usar el mismo proveedor para ambas cosas significa una sola cuenta, una sola clave de API y una sola facturación. `gpt-4o-mini` es además de los modelos más baratos del mercado para una tarea liviana como pulir un mensaje corto. |
| Transcripción de audio | Google Cloud Speech-to-Text, Azure AI Speech, AssemblyAI, Deepgram | Mismo razonamiento: evita sumar un tercer proveedor solo para esta tarea, y Whisper tiene soporte sólido de español. |
| Clonación y síntesis de voz | Google Cloud TTS, Amazon Polly, Microsoft Azure Speech (voces neuronales de alta calidad, pero sin clonación instantánea accesible), Play.ht, Resemble AI, Murf.ai, Coqui (open-source, autoalojable) | ElevenLabs es ampliamente reconocido como líder en calidad de clonación instantánea (qué tanto se parece el clon a la voz real) y en naturalidad multilingüe con buen soporte de español. Las alternativas de las grandes nubes ofrecen TTS neuronal excelente pero clonación de voz limitada o inexistente en sus planes accesibles; las opciones open-source (Coqui) requieren infraestructura propia con GPU que esta app no tiene ni necesita a esta escala. |

**¿Esto ya existe en apps desarrolladas por otras empresas?**
Sí, y es importante ser honesto al respecto: la tecnología de base (clonación de voz, texto-a-voz, edición de texto con IA) no la inventa este proyecto — la proveen ElevenLabs y OpenAI, y ya alimenta productos comerciales conocidos (herramientas de doblaje, dictado, bots de voz de atención al cliente, apps como Descript, Synthesia, Murf, o los propios "Projects" de ElevenLabs). Ningún proyecto de este tamaño construye un modelo de clonación de voz desde cero, y no tendría sentido intentarlo. El aporte real de "Líder Activo" no es la IA en sí, sino la **integración y el flujo de trabajo específico**: llevar a un líder de una empresa ecuatoriana desde "una idea escrita a las apuradas" hasta "un mensaje pulido, con su propia voz clonada, listo para enviar por los canales que su equipo ya usa (WhatsApp, Teams, correo)" en un solo flujo simple, con filtros de seguridad (moderación contra extorsión/insultos) y cumplimiento legal local (LOPDP) incorporados de fábrica — algo que ninguna de esas plataformas de IA ofrece por sí sola, porque son piezas de infraestructura, no un producto de comunicación de liderazgo terminado.

**¿Por qué `gpt-4o-mini` y no un modelo más moderno? ¿Y por qué Whisper y no un modelo de transcripción más moderno?**
Son dos preguntas distintas, con respuestas distintas:

- *Para mejorar texto (`gpt-4o-mini`):* sigue siendo una elección válida porque la tarea (pulir gramática/tono de un mensaje corto) no requiere razonamiento de frontera. Existen sucesores más nuevos (`gpt-5-mini`, `gpt-5.4-mini`), pero no está claro que sean "mejores" para *esta* tarea puntual: `gpt-5-mini` sale un poco más barato en tokens de entrada pero más caro en salida, y `gpt-5.4-mini` cuesta 5-7 veces más que `gpt-4o-mini` por token. Migrar sería trivial (cambiar un string, sin tocar el resto del código), pero el costo/latencia extra no se justifica para una tarea de edición liviana. Es la parte más fácil de actualizar el día que se necesite.
- *Para transcribir (Whisper):* acá sí hay un hallazgo real que vale la pena tener presente para la defensa. OpenAI lanzó en julio de 2026 un modelo de transcripción más nuevo, **`gpt-transcribe`**, que reduce el error de transcripción de forma importante (de ~40% a ~19% de word error rate en Common Voice, midiendo 22 idiomas) y cuesta un 25% menos por minuto que Whisper. **No es un reemplazo directo**: cambia el formato de respuesta — ya no admite `response_format="verbose_json"` con un campo `language` simple (como el que usa `entrenar_voz()` para detectar el idioma de la voz clonada, sección 5.2), sino que devuelve un arreglo `languages: [{"code": "es"}]` — así que migrar implica adaptar ese código, no solo cambiar el nombre del modelo. Es una mejora real identificada y pendiente de evaluar/migrar, no implementada todavía en esta versión.
